...
export function tempDbPathInNewDir() {
    const dir = path.join(os.tmpdir(), `sitrep-test-${crypto.randomBytes(6).toString('hex')}`);
...
export function cleanupDb(dbPath) {
    ...
    } catch { /* directory not empty or already deleted */ }
}
...
export function createTempMigrationsDir(files) {
    const dir = path.join(os.tmpdir(), `migrations-${crypto.randomBytes(6).toString('hex')}`);
