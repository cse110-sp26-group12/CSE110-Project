...
export function getDb() {
    if (!dbInstance) {
        dbInstance = createDatabaseConnection(process.env.DATABASE_PATH || './database/data/sitrep.db');
    }
    return dbInstance;
}

process.on('exit', () => dbInstance?.close());
process.on('SIGINT', () => { dbInstance?.close(); process.exit(0); });
process.on('SIGTERM', () => {dbInstance?.close(); process.exit(0); });