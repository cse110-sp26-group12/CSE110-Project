...
- `add(name, done, todo, blockers)`: creates a standup and pushes it to the store, returns the new entry
- `getAll()`: returns a defensive copy of the standups array
- `toJSON()`: returns all standups serialized to pretty-printed JSON
...
console.log(standupStore.toJSON());
```
...
The factory keeps `submittedAt` in ISO 8601 so the Postgres `timestamptz` column ingests it without conversion. The `id` column is added at the database layer since Phase 1 has no identifier need (in-memory order is enough).
