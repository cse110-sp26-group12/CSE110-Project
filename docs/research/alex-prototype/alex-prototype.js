const app = {
  ...
  get currentProject() {
    ...
    return this.state.projects.find(
      (p) =>
        ...
        p.members.includes(this.state.currentUser),
    );
  },
  ...
  get userInvitations() {
    ...
    return this.state.invitations.filter(
      (i) => i.to === this.state.currentUser && i.status === 'pending',
    );
  },
  ...
  inviteMember(e) {
    ...
    const pending = this.state.invitations.some(
      (i) =>
        i.to === name &&
    ...
    list.innerHTML = invs.length
      ? invs
        ...
        `,
          ...
          .join('')
      : '<p style="text-align: center; color: var(--gray);">Your inbox is empty.</p>';
  },
  ...
  renderTasks() {
    ...
    const activeTasks = this.currentProject.tasks
      ...
      .sort(
        (a, b) => this.priorityMap[b.priority] - this.priorityMap[a.priority],
      );
    ...
    list.innerHTML = activeTasks.length
      ? activeTasks
        ...
        `,
          ...
          .join('')
      : '<p style="text-align: center; color: var(--gray);">No active tasks. Add one to get started!</p>';
  },

  renderStandups() {
    const feed = document.getElementById('standupFeed');
    feed.innerHTML = this.currentProject.standups.length
      ? this.currentProject.standups
          .map(
            (s) => `
        ...
        `,
          ...
          .join('')
      : '<p style="text-align: center; color: var(--gray);">No stand-up updates yet.</p>';

    const select = document.getElementById('standupMember');
    select.innerHTML = this.currentProject.members
  ...
  renderMembers() {
    ...
    list.innerHTML = this.currentProject.members
      .map(
        (m) => `
    ...
    list.innerHTML = completedTasks.length
      ? completedTasks
        ...
        `,
          ...
          .join('')
      : '<p style="text-align: center; color: var(--gray);">No completed tasks yet.</p>';
  },
  ...
  renderEmptyState() {
    ...
    views.forEach((v) => {
      ...
      if (el)
        el.innerHTML =
          '<p style="text-align: center; color: var(--gray);">You are not a member of any projects. Create one to get started!</p>';
    });
