...
export const ui = {
  ...
  updateTime() {
    const now = new Date();
    this.elements.systemTime.textContent =
  ...
  populateMemberSelect() {
    ...
    this.elements.memberSelect.innerHTML =
  ...
  renderStats() {
    ...
    const totalMood = reports.reduce(
      (acc, r) => acc + parseInt(r.sentiment),
      0,
    );
    const avgMood = reports.length
      ? (totalMood / reports.length).toFixed(1)
      : '0.0';
    const avgAgentLoad =
      agents.reduce((acc, a) => acc + a.load, 0) / agents.length;

    this.elements.stats.active.querySelector('.value').textContent =
      store.getMembers().length;
    this.elements.stats.blocked.querySelector('.value').textContent =
      blockedCount;
    this.elements.stats.mood.querySelector('.value').textContent = avgMood;
    this.elements.stats.agents.querySelector('.value').textContent =
      Math.round(avgAgentLoad) + '%';
  },

  renderReports() {
    const reports = store.getReports();
    this.elements.reportsContainer.innerHTML = reports
      .map((r) => {
  ...
  renderAgents() {
    const agents = store.getAgents();
    this.elements.agentPulseContainer.innerHTML = agents
      .map(
        (a) => `
  ...
  renderRoster() {
    ...
    this.elements.rosterContainer.innerHTML = members
      .map((m) => {
        const lastReport = reports.find((r) => r.memberId === m.id);
        const statusClass = lastReport
          ? lastReport.blockers
            ? 'blocked'
            : 'operational'
          : 'offline';
  ...
  renderWarRoom() {
    ...
    this.elements.availabilityMatrix.innerHTML = members
      .map((m) => {
        const lastReport = reports.find((r) => r.memberId === m.id);
