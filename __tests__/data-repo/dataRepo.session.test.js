...
describe('sessionRepo', () => {
    ...
    let tokenSeq = 0;
    function seedSession({ user_id, token,
                           created_at = '2026-01-01T00:00:00.000Z',
                           expires_at, revoked_at = null,
                           user_agent = null, ip_address = null }) {
        tokenSeq += 1;
        const result = db.prepare(`
            INSERT INTO user_sessions (token, user_id, created_at, expires_at, user_agent, ip_address, revoked_at)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `).run(token ?? `tok-${tokenSeq}`, user_id, created_at,
    ...
    describe('create', () => {
        ...
        it('persists the row to the database', () => {
            const userId = seedUser();
            const s = sessionRepo.create({ token: 'tok-xyz', user_id: userId, expires_at: FUTURE });
            const row = db.prepare('SELECT * FROM user_sessions WHERE id = ?').get(s.id);
            expect(row.token).toBe('tok-xyz');
            expect(row.user_id).toBe(userId);
        });

        it('defaults user_agent and ip_address to null when omitted', () => {
            const userId = seedUser();
            const s = sessionRepo.create({ token: 'tok-min', user_id: userId, expires_at: FUTURE });
        ...
        it('sets created_at as an ISO timestamp and leaves revoked_at null', () => {
            const userId = seedUser();
            const s = sessionRepo.create({ token: 'tok-ts', user_id: userId, expires_at: FUTURE });
        ...
        it('throws on a duplicate token', () => {
            const userId = seedUser();
            sessionRepo.create({ token: 'dup', user_id: userId, expires_at: FUTURE });
            expect(() => {
                sessionRepo.create({ token: 'dup', user_id: userId, expires_at: FUTURE });
            }).toThrow(/UNIQUE/);
        });
        ...
        it('throws when user_id references a nonexistent user', () => {
            expect(() => {
                sessionRepo.create({ token: 'tok-ghost', user_id: 99999, expires_at: FUTURE });
    ...
    describe('findById', () => {
        it('returns the matching session', () => {
            const userId = seedUser();
            const id = seedSession({ user_id: userId, token: 'find-me', expires_at: FUTURE });
            const s = sessionRepo.findById(id);
            expect(s.id).toBe(id);
            expect(s.token).toBe('find-me');
        });
    ...
    describe('findByToken', () => {
        it('returns the matching session', () => {
            const userId = seedUser();
            seedSession({ user_id: userId, token: 'by-token', expires_at: FUTURE });
            const s = sessionRepo.findByToken('by-token');
            expect(s).toBeDefined();
            expect(s.token).toBe('by-token');
        });

        it('returns undefined when no such token exists', () => {
            expect(sessionRepo.findByToken('nope')).toBeUndefined();
        });

        it('returns a revoked session (unfiltered)', () => {
            const userId = seedUser();
            seedSession({ user_id: userId, token: 'revoked-tok', expires_at: FUTURE, revoked_at: REVOKED_TS });
            const s = sessionRepo.findByToken('revoked-tok');
            expect(s).toBeDefined();
        ...
        it('returns an expired session (unfiltered)', () => {
            const userId = seedUser();
            seedSession({ user_id: userId, token: 'expired-tok', expires_at: PAST });
            const s = sessionRepo.findByToken('expired-tok');
            expect(s).toBeDefined();
    ...
    describe('findValidByToken', () => {
        it('returns a session that is neither revoked nor expired', () => {
            const userId = seedUser();
            seedSession({ user_id: userId, token: 'valid', expires_at: FUTURE, revoked_at: null });
        ...
        it('returns undefined for an expired-but-not-revoked session', () => {
            const userId = seedUser();
            seedSession({ user_id: userId, token: 'expired', expires_at: PAST, revoked_at: null });
            expect(sessionRepo.findValidByToken('expired')).toBeUndefined();
        });

        it('returns undefined for a revoked-but-not-expired session', () => {
            const userId = seedUser();
            seedSession({ user_id: userId, token: 'revoked', expires_at: FUTURE, revoked_at: REVOKED_TS });
        ...
        it('returns undefined for a session that is both revoked and expired', () => {
            const userId = seedUser();
            seedSession({ user_id: userId, token: 'dead', expires_at: PAST, revoked_at: REVOKED_TS });
    ...
    describe('listByUser', () => {
        it('returns all of a user\'s sessions newest-first by default', () => {
            const userId = seedUser();
            seedSession({ user_id: userId, token: 'old', expires_at: FUTURE, created_at: '2026-01-01T00:00:00.000Z' });
            seedSession({ user_id: userId, token: 'mid', expires_at: FUTURE, created_at: '2026-02-01T00:00:00.000Z' });
            seedSession({ user_id: userId, token: 'new', expires_at: FUTURE, created_at: '2026-03-01T00:00:00.000Z' });

            const sessions = sessionRepo.listByUser(userId);
            expect(sessions.map(s => s.token)).toEqual(['new', 'mid', 'old']);
        });

        it('includes revoked and expired sessions by default', () => {
            const userId = seedUser();
            seedSession({ user_id: userId, token: 'valid',   expires_at: FUTURE });
            seedSession({ user_id: userId, token: 'expired', expires_at: PAST });
            seedSession({ user_id: userId, token: 'revoked', expires_at: FUTURE, revoked_at: REVOKED_TS });
        ...
        it('returns only valid sessions when onlyValid is true', () => {
            ...
            expect(valid[0].token).toBe('valid');
        });

        it('does not return another user\'s sessions', () => {
            const userA = seedUser();
            const userB = seedUser();
            seedSession({ user_id: userA, token: 'a-tok', expires_at: FUTURE });
            seedSession({ user_id: userB, token: 'b-tok', expires_at: FUTURE });

            const sessions = sessionRepo.listByUser(userA);
            expect(sessions).toHaveLength(1);
            expect(sessions[0].token).toBe('a-tok');
        });
    ...
    describe('countValidByUser', () => {
        it('counts only valid sessions', () => {
            const userId = seedUser();
            seedSession({ user_id: userId, token: 'v1', expires_at: FUTURE });
            seedSession({ user_id: userId, token: 'v2', expires_at: FUTURE });
            seedSession({ user_id: userId, token: 'expired', expires_at: PAST });
            seedSession({ user_id: userId, token: 'revoked', expires_at: FUTURE, revoked_at: REVOKED_TS });
        ...
        it('does not count another user\'s valid sessions', () => {
            const userA = seedUser();
            const userB = seedUser();
            seedSession({ user_id: userA, token: 'a', expires_at: FUTURE });
            seedSession({ user_id: userB, token: 'b', expires_at: FUTURE });
    ...
    describe('revoke', () => {
        ...
        it('makes a previously-valid session invalid', () => {
            const userId = seedUser();
            const id = seedSession({ user_id: userId, token: 'will-revoke', expires_at: FUTURE });
    ...
    describe('revokeByToken', () => {
        it('revokes the session matching the token', () => {
            const userId = seedUser();
            seedSession({ user_id: userId, token: 'tok-logout', expires_at: FUTURE });
        ...
        it('does not change revoked_at on an already-revoked token', () => {
            const userId = seedUser();
            seedSession({ user_id: userId, token: 'already', expires_at: FUTURE, revoked_at: REVOKED_TS });
    ...
    describe('revokeAllForUser', () => {
        it('revokes all currently-valid sessions and returns the count', () => {
            ...
            seedSession({ user_id: userId, token: 'v1', expires_at: FUTURE });
            seedSession({ user_id: userId, token: 'v2', expires_at: FUTURE });
            seedSession({ user_id: userId, token: 'v3', expires_at: FUTURE });
        ...
        it('does not re-revoke already-revoked sessions (count reflects only newly revoked)', () => {
            const userId = seedUser();
            seedSession({ user_id: userId, token: 'fresh', expires_at: FUTURE });
            seedSession({ user_id: userId, token: 'already', expires_at: FUTURE, revoked_at: REVOKED_TS });
    ...
    describe('purgeDeadSessions', () => {
        it('deletes expired and revoked sessions, keeping valid ones', () => {
            const userId = seedUser();
            const validId = seedSession({ user_id: userId, token: 'valid', expires_at: FUTURE });
            seedSession({ user_id: userId, token: 'expired', expires_at: PAST });
            seedSession({ user_id: userId, token: 'revoked', expires_at: FUTURE, revoked_at: REVOKED_TS });
            seedSession({ user_id: userId, token: 'dead',    expires_at: PAST, revoked_at: REVOKED_TS });
        ...
        it('purges across multiple users', () => {
            const userA = seedUser();
            const userB = seedUser();
            seedSession({ user_id: userA, token: 'a-dead', expires_at: PAST });
            seedSession({ user_id: userB, token: 'b-dead', expires_at: PAST });
            seedSession({ user_id: userA, token: 'a-valid', expires_at: FUTURE });
        ...
        it('cascades correctly: deleting a user removes their sessions too', () => {
            ...
            const userId = seedUser();
            const id = seedSession({ user_id: userId, token: 'tok', expires_at: FUTURE });
